...
export default class BronzeHelper {
   ...
   *
   * @param {string} id - Possible file extensions will be ignored so that a
   *                      file path can be directly used.
   */
  getImage(id: string, profile: string) {
    // Ensure that we have an image ID and not a source path by removing the
    // file extension.
    let pathO = path.parse(id);
    id = path.join(pathO.dir, pathO.name);
